...
function Extension() {
  ...
  const orderName = shopify.order.value?.name;
    ...
    async function loadSurvey() {
      try {
        const response = await authorizedFetch(
          `/api/survey/customer-account?shopifyOrderGid=${encodeURIComponent(orderId)}`,
        );
  ...
  async function submit() {
    ...
    try {
      const response = await authorizedFetch("/api/survey/customer-account", {
        method: "POST",
